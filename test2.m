clearvars -except t Fluxes_Input o F it
...
%     error('!!!')
% end
% Fluxes_Input=randi(nloc,1,nloc);
...
nvars=nass*ncycles + nass*npos*(ncycles-1) + 1 + ncycles;
%find how many constraints
nineqs = nass*(nass-1)*ncycles + nass*(nass-1);
...
for i = 1:nvars
    ...
    elseif i <= nass*ncycles+nass*npos*(ncycles-1)
        ctype(end+1) = 'B'; 
    elseif i <= nass*ncycles+nass*npos*(ncycles-1)+1
        ctype(end+1) = 'C';
    else
        ctype(end+1) = 'C'; 
    end
        ...
        for ip=1:nass
            if i~=ip
                for l=1:k
                    ...
                    Aineq_j(idx_Aineq)=(l-1)*nass+i;
                    Aineq_v(idx_Aineq)=1;
                    idx_Aineq = idx_Aineq+1;
                    
                    Aineq_i(idx_Aineq)=constraint_idx;
                    Aineq_j(idx_Aineq)=(l-1)*nass+ip;
                    Aineq_v(idx_Aineq)=-1;
                    idx_Aineq = idx_Aineq+1;
                end
                Aineq_i(idx_Aineq)=constraint_idx;
                Aineq_j(idx_Aineq)=nass*ncycles + nass*npos*(ncycles-1) + 1 + k;
                Aineq_v(idx_Aineq)=-1;
                idx_Aineq = idx_Aineq+1;
...
% Upper bound
constraint_idx = nass*(nass-1)*ncycles + 1;
...
%objective
c=zeros(nvars,1);
c(nass*ncycles+nass*npos*(ncycles-1)+1)=1e3;
c(nass*ncycles+nass*npos*(ncycles-1)+1+[1:ncycles])=1;
...
%% Post processing
solutionvector=round(solutionvector,6);
Fluence(:,1)=Fluxes(:,1);
...
for k=2:ncycles
    for i=1:nass
        for j=1:npos
            delta(i,j,k)=solutionvector(nass*ncycles+(k-2)*nass*npos+(i-1)*npos+j);
        end
        if sum(delta(i,:,k))~=1
            error('Error on deltas')
        end
    end
end
for i=1:nass
    for k=2:ncycles
        ...
        pos_in_batch(i,k)=find(delta(i,:,k)==1);
...
Max_steps=[solutionvector(nass*ncycles+nass*npos*(ncycles-1)+1+[1:ncycles])]';
Max=solutionvector(nass*ncycles+nass*npos*(ncycles-1)+1);
save Shuffling
if status~=1
