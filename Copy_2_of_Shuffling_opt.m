...
for k=1:ncycles
    for i=1:nass %assembly
        for ip=1:nass
            Aineq_i(idx_Aineq)=(k-1)*nass*nass+(i-1)*nass+ip;
            ...
            Aineq_i(idx_Aineq)=(k-1)*nass*nass+(i-1)*nass+ip;
            Aineq_j(idx_Aineq)=nass*nloc*ncycles+k;
            Aineq_v(idx_Aineq)=-1;
            idx_Aineq = idx_Aineq+1;
            
            bineq((k-1)*nass*nass+(i-1)*nass+ip) =0;
        end
    end
end
...
for k=1:ncycles
    for j = 1:nloc
        for i = 1:nass
            Aineq_i(idx_Aineq) = nass*nass*ncycles+(k-1)*nloc+j;
            Aineq_j(idx_Aineq) = nass*ncycles+(k-1)*nass*nloc+(i-1)*nloc+j;
            Aineq_v(idx_Aineq) = 1;
            idx_Aineq = idx_Aineq+1;
        end
        bineq(nass*nass*ncycles+(k-1)*nloc+j) = 1;
    end
end
...
for k=1:ncycles
    for i = 1:nass
        ...
        for l=1:k
            for j = 1:nloc
                Aeq_i(idx_Aeq) = (k-1)*nass+i;
                Aeq_j(idx_Aeq) = nass*ncycles+(l-1)*nass*nloc+(i-1)*nloc+j;
    ...
    for i=1:nass
        for j=1:nloc
            if Batches(j)~=Shuf_scheme(k)
                ...
                Aeq_j(idx_Aeq) = nass+(k-1)*nass*nloc+(i-1)*nloc+j;
                Aeq_v(idx_Aeq) = 1;
                idx_Aeq = idx_Aeq+1;
            end
        end
...
if status==1
    for k=1:ncycles
        max_dif(k)=solutionvector(ncycles*nass*nloc+k);
        for i=1:nass
            for j=1:nloc
                deltas(i,j,k)=solutionvector((k-1)*nass*nloc+(i-1)*nloc+j);
            end
    ...
    for i=1:nass
        for k=1:ncycles
            ...
            Acum_Fluence(i,k)=sum(Fluence(i,:));
        end
